...
class main():
    ...
    def run_initial(self):
        ...
        try:
            G16(input_sdf=self._input, 
                ...
                if_d3=self.if_d3, 
                if_chk=self.if_chk,
                define_charge=self.charge).run(prefix="opt")
            G16(input_sdf=self._input, 
                mode="sp", 
                ...
                if_d3=self.if_d3, 
                if_chk=self.if_chk,
                define_charge=self.charge).run(prefix="sp")
        except Exception as e:
    ...
    def run_rotation(self, opt_run_done_log):
        ...
        G16(input_rdmol_obj=mol_set, 
            ...
            if_d3 = self.if_d3,
            if_chk = self.if_chk, 
            define_charge=self.charge).run(prefix=get_this_sys_prefix)
    ...
    def result_2D_scan(self, scan_done_log):
        ...
        ## there you have _log_dih
        if _log_dih:
            _log_dih_sort = sorted(_log_dih.items(), key=lambda x: int(x[0].split("_")[-1]))
            _ene_min = min([cc[-1] for cc in _log_dih.values()])
